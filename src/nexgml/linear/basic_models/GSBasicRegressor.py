...
from ...amo import forlinear                 # For specific computation operations
...
class BasicRegressor:
    ...
    def __init__(
            ...
            alpha: float=0.0001, 
            l1_ratio: float=0.5, 
            loss: Literal["mse", "mae", "epsilon_insensitive", "squared_epsilon_insensitive"] | None="mse",
        ...
        ## Args:
            ...
            **loss**: *{'mse', 'mae', 'epsilon_insensitive', 'squared_epsilon_insensitive'}, default='mse'*
        ...
        if loss not in ('mse', 'mae', 'epsilon_insensitive', 'squared_epsilon_insensitive'):
            raise ValueError(f"Invalid loss argument, {loss}. Choose from 'mse', 'mae', 'epsilon_insensitive', or 'squared_epsilon_insensitive'.")
    ...
    def _calculate_grad(self, X: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
        ...
        # Epsilon Insensitive loss gradient
            ...
            grad_w, grad_b = forlinear.epsilon_insensitive_deriv(X, error, self.epsilon, self.intercept)

        # Squared Epsilon Insensitive loss gradient
        elif self.loss == 'squared_epsilon_insensitive':
            grad_w, grad_b = forlinear.squared_epsilon_insensitive_deriv(X, error, self.intercept)
