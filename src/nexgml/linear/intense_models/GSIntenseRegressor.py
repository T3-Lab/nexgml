...
from ...amo import forlinear                 # For specific numerical computations
from warnings import warn                    # For warning messages
from ...metrics import r2_score              # For R2 score calculation
from ...guardians import safe_array          # For numerical stability
...
class IntenseRegressor:
    ...
    MSE, MAE, and SmoothL1 loss functions are available.
    ...
    def __init__(
        ...
        alpha: float=0.0001, 
        l1_ratio: float=0.5, 
        loss: Literal["mse", "mae", "smoothl1"] | None="mse", 
        ...
        ## Args:
            ...
            **loss**: *{'mse', 'mae', 'smoothl1', 'epsilon_insensitive', 'squared_epsilon_insensitive'}, default='mse'*
            ...
            **epsilon**: *float, default: 1e-10*
            Small constant for numerical stability in learning rate calculations and Adam updates.
        ...
        if loss not in {'mse', 'mae', 'smoothl1', 'epsilon_insensitive', 'squared_epsilon_insensitive'}:
            raise ValueError(f"Invalid loss argument, {loss}. Choose from 'mse', 'mae', 'smoothl1', 'epsilon_insensitive', or 'squared_epsilon_insensitive'")
    ...
    def _calculate_loss(self, y_true: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        # Epsilon Insensitive loss function
            ...
            loss = forlinear.epsilon_insensitive(y_true, y_pred, self.epsilon)

        # Squared Epsilon Insensitive loss function
        elif self.loss == 'squared_epsilon_insensitive':
            loss = forlinear.epsilon_insensitive(y_true, y_pred, self.epsilon, squared=True)
    ...
    def _calculate_grad(self, X: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
        ...
        # Epsilon Insensitive loss gradient
            ...
            grad_w, grad_b = forlinear.epsilon_insensitive_deriv(X, residual, self.epsilon, self.intercept)

        # Squared Epsilon Insensitive loss gradient
        elif self.loss == 'squared_epsilon_insensitive':
            grad_w, grad_b = forlinear.squared_epsilon_insensitive_deriv(X, residual, self.intercept)
